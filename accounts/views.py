...
def logout_user(request):
    logout(request)
    messages.info(
        request, 'You have successfully logged out. Login again anytime to access your dashboard')
...
def admin_dashboard(request):
    ...
    returned_books = lmdl.Borrowedbook.objects.filter(
        book_returned=True).count()
...
def student_registration(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            new_user = mdl.User(username=new_user_id, email=user_email,
                                is_administrator=user_is_admin, is_student=user_is_student)
            try:
...
def student_dashboard(request):
    ...
    my_book = lmdl.Borrowedbook.objects.filter(
        book_returned=False, borrower=request.user).count()
    my_return_books = lmdl.Borrowedbook.objects.filter(
        borrower=request.user, book_returned=True, confirm_returned_book=True).count()
    over_due_books = lmdl.Borrowedbook.objects.filter(
        borrower=request.user).all()
    over_due = 0
...
def my_borrowed_books(request):
    my_books = lmdl.Borrowedbook.objects.order_by(
        '-book_returned').filter(borrower=request.user).all()
...
def my_read_books(request):
    read_books = lmdl.Borrowedbook.objects.order_by(
        '-date_returned').filter(borrower=request.user, book_returned=True).all()
...
def my_returned_books(request):
    returned_books = lmdl.Borrowedbook.objects.filter(
        borrower=request.user, book_returned=True, confirm_returned_book=True).all()
...
def return_a_book(request, book_id):
    my_borrowed_book = get_object_or_404(
        lmdl.Borrowedbook, id=book_id, borrower=request.user)
...
def request_a_book(request, book_id):
    ...
    if book.is_avaliable():
        if (lmdl.Requestedbook.objects.filter(book=book, requested_user=request.user, request_book=True)).first() is None:
            ...
            messages.success(
                request, 'Your Request For The Book Was Successful.')
        ...
        messages.error(
            request, 'The Book You Request Is Not Avaliable At The Moment.')
        return redirect('accounts:library-books')


def save_requested_book(book, user):
    lmdl.Requestedbook.objects.create(
        book=book, requested_user=user, request_book=True)
    book.number_in_stock -= 1
    book.save()


def disable_my_book_request(request, book_id):
    requested_book = get_object_or_404(
        lmdl.Requestedbook, id=book_id, requested_user=request.user)
...
def my_requested_books(request):
    my_books = lmdl.Requestedbook.objects.filter(
        requested_user=request.user, request_book=True).all()
    context = {'my_books': my_books}
